...
def create_cooling_laser(
    ...
    f_heat_prep = (
        f"variable targetT equal {T_d}\n"
        f"variable curr_temp equal temp\n"
    )

    f_heat_add = (
        f'every 1 "if \'${{curr_temp}} < ${{targetT}}\' then \'fix hterm {gid} temp/rescale 1 ${{targetT}} ${{targetT}} 0 1\' else \'fix hterm {gid} temp/rescale 1 ${{targetT}} ${{targetT}} 0 0\'"\n'
