...
def generate_side_effect(time_sequence, current_timeblock_num):
...
def mock_configur():
    configur = MagicMock()
    configur.get.side_effect = generate_side_effect("[[0.1, 10], [0.2, 20]]", "0")
    return configur


@pytest.fixture
def mock_dump_dir():
    return lambda setup: "/mock/path/"
...
def test_evolve_happy_path(
    ...
    mock_configur.get.side_effect = generate_side_effect(
        time_sequence, current_timeblock_num
    )
