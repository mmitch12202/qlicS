...
def mock_configur():
    configur = MagicMock()
    configur.get.side_effect = generate_side_effect("[[0.1, 10], [0.2, 20]]", "0", "/mock/path/")
    return configur


@pytest.fixture
def mock_dump_dir():
    return lambda setup: "/mock/path/" # TODO: this mock path is hardcoded in two places - fix
...
def test_evolve_happy_path(
    ...
    ), patch("builtins.open", mock_open(read_data="data")), patch("qlicS.config_controller.configur", mock_configur):
        result = evolve()
