...
@pytest.mark.parametrize(
        "iter_config, expected_calls",
        [
            (
                [
                ('scan_objects', '["modulation_0", "scattering_laser", "cooling_laser_0"]'),
...
def test_iter_appending(iter_config, expected_calls):
    ...
    with patch("qlicS.exp_sequence_controller.configur.items", return_value=iter_config), \
        patch("qlicS.exp_sequence_controller.give_command_mapping", return_value={
        ...
        all_append_args = [call[0][0]._extract_mock_name() for call in mock_s.append.call_args_list]
